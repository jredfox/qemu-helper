...
vmlinuz_path="$(printf '%s' "$results_sorted" | grep -Ei '(hwe-)?(vmlinuz|zImage|uImage|bzImage|Image|linux|vmlinux|kernel\.ubuntu|kernal\.ubuntu)(\.ubuntu)?(-rt|-cloud|-virt|-vm|-generic|-lts|-hwe){0,7}(\.efi|\.gz|\.lz|\.img|\.tar\.gz|\.cpio\.gz)?$')"
initrd_path="$(printf '%s' "$results_sorted" | grep -Ei '(hwe-)?(initrd|uInitrd|initramfs|initramfs-linux)(\.ubuntu)?(-rt|-cloud|-virt|-vm|-generic|-lts|-hwe){0,7}(\.efi|\.gz|\.lz|\.img|\.tar\.gz|\.cpio\.gz)?$')"
echo "kernals:"
echo "$vmlinuz_path"
echo "initrd:"
echo "$initrd_path"